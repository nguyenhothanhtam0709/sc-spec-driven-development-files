#!/usr/bin/env python3
"""Maintain CHANGELOG.md from git commit history, scoped to this project directory."""
...
# This project lives in a subdirectory of a shared repo (one repo holds many
# lesson projects), so every `git log` call is scoped with `-- .` to only
# include commits that touched files under the project root. In a repo
# dedicated to a single project this is a no-op.


def git_log(since_date=None):
    """Return commits touching this directory as {date: [subject, ...]}."""
